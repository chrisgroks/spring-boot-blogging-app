...
import com.reportingdashboard.graphql.DgsConstants.USERPAYLOAD;
import com.reportingdashboard.graphql.types.User;
...
public class MeDatafetcher {
  ...
  public DataFetcherResult<User> getMe(
    ...
    com.reportingdashboard.core.user.User user = (com.reportingdashboard.core.user.User) authentication.getPrincipal();
    UserData userData =
  ...
  public DataFetcherResult<User> getUserPayloadUser(
      DataFetchingEnvironment dataFetchingEnvironment) {
    com.reportingdashboard.core.user.User user = dataFetchingEnvironment.getLocalContext();
    User result =
        User.newBuilder()
            .email(user.getEmail())
